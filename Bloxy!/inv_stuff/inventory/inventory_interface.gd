...
func _physics_process(delta: float) -> void:
	...
	if external_inventory_owner and external_inventory_owner.pos.distance_to(PlayerManager.get_global_position()) > 6:
		force_close.emit()
...
func _on_inventory_interface_visibility_changed():
	if not visible and grabbed_slot_data:
		pass #put item back in slot when you are still holding it and you hide inv
